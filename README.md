...
update:20240310
说明:
### 1.macOS 14.4 之前的版本,请使用 20240110 版,主要是英特尔的 wifi 驱动在 14.4 出现变动,使用之前的版本在 10.14 无法进入系统.
### 2.macOS 版本 14.4 ,直接使用 20240310 版,英特尔驱动已更新到 14.4 专用版,不会出现无法进入系统的情况,同时在安装是先更改 config.plist-misc-Security-SecureBootmodel为 Disable状态,安装成功后再更改为 Default 状态.
### 3.由 macOS14.4 版本之前升级到 14.4 版本,先使用 20240110 版本下载并安装更新,然后更换 EFI 为 20240310 版,再重启更新,否则无法进入系统.
重启安装更新前,建议先禁用Airportltlwm\InterlBluetoothFirmware\InterBTPatcher等 3 个 kext,避免出现无法更新状态.以上建议,也适用于其他机型,供参考.
